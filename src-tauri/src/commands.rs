...
pub async fn index_directory(
    ...
    // Get custom model path from config for embedding model
    let custom_model_path = config.model_path.clone();

    // Try to load embedding model once (shared across all chunks)
    let embedding_model = match crate::embeddings::EmbeddingModel::new(custom_model_path.as_deref()) {
        Ok(model) => {
